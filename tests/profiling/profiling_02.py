...
print(
    f"{np.array(arr_acc_dnf).mean():0.3f}           {np.array(arr_acc_classi).mean():0.3f}              "
    f"{np.array(arr_test_size).mean():0.1f}/{i1.shape[1]}            {np.array(arr_conseq).mean():0.3f}"  # type: ignore
    f"     {np.array(arr_equiv).mean():0.3f}    {np.array(arr_time).mean():0.6f}"
)
